...
############################################################
# takes a string
# and returns quoted string for safe use with DBs. Hardly used,
# as we use parameterization to ensure for quoted text.
sub db_quote {
    my ( $line ) = @_ ;
    state $dbh = MyDB::db_connect( $Database ) ;
    return $dbh->quote( $line ) or croak $dbh->errstr ;
    }
